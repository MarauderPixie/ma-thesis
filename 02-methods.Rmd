...
1. a control condition, where participants neither encounter rule language nor a simple rule at the beginning
...
Futhermore, all models will use weakly informative priors. @Gelman2020 recommend $student(df, mean = 0, sd = 1)$ where $3 \leq df \leq 7$, since the often used Cauchy-Distribution (that is, $student(1, mean = 0, sd = 2.5)$) has too thick tails and a strict Gaussian might be too informative in the case at hand.  
Finally, Bayes Factors for model comparisons will be obtained via bridge sampling [@Gronau2020]. To obtain as robust BFs as possible, each model will have 4 MCMC chains with 20000 samples and 4000 warm-up samples each (see @Schad2021).

**Hypotheses 1** -- The dependent variable is the number of extrapolations of the nine untrained/incomplete stimuli (_k_ extrapolations of _n_ = 9 stimuli). Hypothesized model comparisons are:

- **H1.1:** $M_{instructions} > M_{null}$
- **H1.2:** $M_{rule order} > M_{null}$
- **H1.3:** $M_{instructions : rule order} \approx M_{instructions} \approx M_{rule order} > M_{null}$
